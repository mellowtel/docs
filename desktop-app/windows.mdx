...
- A Mellowtel account and **Integration ID** (get yours from the [dashboard](https://www.mellowtel.com/mellowtel-auth/)). Each desktop app gets its own unique Integration ID.
- A NuGet access token (revealed below in the installation section).
...
<configuration>
  <packageSources>
    ...
    <add key="github-mellowtel" value="https://nuget.pkg.github.com/mellowtel-inc/index.json" />
  </packageSources>
...
Replace `YOUR_PROVIDED_TOKEN` with the install token below. Click to reveal, then use the copy icon on the code block to copy it to your clipboard.
...
var mellowtel = new Mellowtel("YOUR_INTEGRATION_ID");

// Check if user has already opted in
if (!mellowtel.GetOptInStatus())
{
    // Show your consent dialog first, and ONLY call OptIn() if the user agrees
...
<Note>
  Replace `YOUR_INTEGRATION_ID` with the Integration ID from your Mellowtel dashboard. `ShowConsentDialog()` is your own UI that returns `true` only when the user explicitly agrees (see [User Consent](#user-consent) below).
</Note>
...
<Tabs>
  <Tab title="Console App">
    ...
    class Program
    {
        ...
        {
            using var mellowtel = new Mellowtel("YOUR_INTEGRATION_ID");
  ...
  <Tab title="WPF App">
    ...
    public partial class MainWindow : Window
    {
        ...
        {
            _mellowtel = new Mellowtel("YOUR_INTEGRATION_ID");
