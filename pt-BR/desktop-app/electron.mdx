...
title: "Integre o Mellowtel no seu App Electron"
icon: "bolt"
platforms:
  - desktop app
...
og:description: "A configuração do Mellowtel Electron SDK abrange autenticação no registro npm, instalação, diálogos de consentimento, configurações, contadores de requisição, comportamento de ciclo de vida e solução de problemas."
...
twitter:description: "A configuração do Mellowtel Electron SDK abrange autenticação no registro npm, instalação, diálogos de consentimento, configurações, contadores de requisição, comportamento de ciclo de vida e solução de problemas."
---
...
### 1. Configure a Autenticação npm
...
- `requestConsent(window, incentive)` renderiza uma **caixa de mensagem nativa do Electron** ancorada à `BrowserWindow` que você passa. O segundo argumento é o título proeminente do diálogo (por exemplo, `"Ganhe 3 meses grátis"`), mostrado acima de uma explicação fixa do que o Mellowtel faz. Resolve para `true` se o usuário aceitar, `false` se o usuário recusar ou fechar o diálogo, e `undefined` se o consentimento já estiver registrado. O SDK persiste automaticamente a decisão de opt-in, então você não precisa chamar `optIn()` depois.
...
1. **Use o diálogo nativo embutido** via `requestConsent(window, incentive)`. Este é o caminho mais rápido e é o que o trecho acima mostra. Ele renderiza um `dialog.showMessageBox` nativo do Electron com seu texto de incentivo como título e persiste automaticamente a decisão do usuário.
...
- `optIn(): Promise<void>` marca o usuário como optado sem mostrar um diálogo. Use isso apenas depois de coletar consentimento através de sua própria UI.
...
As contagens de requisição são persistidas localmente através de `electron-store` e sobrevivem a reinicializações do app. Exiba-as na sua própria UI se quiser mostrar aos usuários o impacto do seu opt-in.
...
Se você quiser desconectar o Mellowtel no meio da sessão (por exemplo, quando um usuário alterna uma configuração de "pausa" no seu app), chame `optOut()`. Isso limpa tanto o sinalizador de opt-in quanto fecha a conexão ativa. Para reconectar, chame `optIn()` seguido de `init()`.

## Persistência de Consentimento

O estado de opt-in é armazenado no caminho de configuração padrão do `electron-store` da plataforma:
...
<AccordionGroup>
  ...
  <Accordion title='Erro "Não autorizado" ou 401 durante a instalação'>
    1. Verifique se o token em `.npmrc` está correto e ativo. Se estiver em dúvida, solicite um novo token em [info@mellowtel.com](mailto:info@mellowtel.com).
  ...
  <Accordion title="O diálogo de consentimento nunca aparece">
    1. Certifique-se de chamar `requestConsent` **depois** que `app.whenReady()` foi resolvido e com uma referência válida e não destruída de `BrowserWindow`.
  ...
  <Accordion title='"init() executa sem erros, mas nada acontece"'>
    Isso é por design. `init()` retorna silenciosamente cedo quando o usuário não optou. Verifique `getOptInStatus()` para confirmar. Se retornar `undefined` ou `false`, execute `requestConsent` primeiro. Note que o log interno "Usuário não optou" é suprimido quando `disableLogs` é deixado no padrão (veja "Logs estão silenciosos" abaixo), então o terminal não dá sinal de qualquer forma até você alterar esse sinalizador.
  </Accordion>
