...
canonical: "https://mellowtel.com/docs/pt-BR/browser-plugins/optional-integrations/meucci"
og:title: "Guia de Configuração do Módulo Meucci da Mellowtel para Plugins de Navegador"
og:description: "A configuração do módulo Meucci da Mellowtel explica a instalação de pacotes, builds padrão e Plasmo, recursos de manifesto e configuração de initContentScript."
og:url: "https://mellowtel.com/docs/pt-BR/browser-plugins/optional-integrations/meucci"
twitter:card: "summary"
twitter:title: "Guia de Configuração do Módulo Meucci da Mellowtel para Plugins de Navegador"
twitter:description: "A configuração do módulo Meucci da Mellowtel explica a instalação de pacotes, builds padrão e Plasmo, recursos de manifesto e configuração de initContentScript."
---

<Warning>
    Este recurso requer a versão 1.6.2 ou superior do Mellowtel. Esta é uma mudança significativa em relação às versões anteriores.
</Warning>

A Mellowtel oferece um recurso opcional que permite lidar com mais solicitações e, portanto, ganhar mais. Este recurso é chamado "Meucci".
...
### 2. Atualize sua configuração do webpack
...
const waitForDirectory = async (dir, timeout = 10000) => {
  ...
  while (!fs.existsSync(dir)) {
    if (Date.now() - start > timeout) {
      throw new Error(`Tempo esgotado esperando pelo diretório: ${dir}`)
    }
...
async function buildMeucci() {
  ...
  await Promise.all(directories.map(async (dir) => {
    try {
      ...
      console.log(`Ignorando ${dir} - ${err.message}`)
    }
  }))
...
2. Ou anexá-lo ao seu script de build existente com `&`: `"build": "seu-comando-de-build-existente & npm run build:meucci"`
...
Se você também tiver o Pascoli habilitado, pode usar o seguinte:
