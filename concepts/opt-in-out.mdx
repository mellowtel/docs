...
1) Creating your own opt-in/opt-out elements. If users agree you can activate them on the Network by calling the `optIn()` method. If they don't agree, you call the `optOut()` method.

2) Using the recommended implementation with the pre-built page provided by Mellowtel. You can simply call the `generateAndOpenOptInLink()` method.
